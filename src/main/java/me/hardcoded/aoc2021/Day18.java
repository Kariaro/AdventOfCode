...
public class Day18 extends DayBase {
	...
	public Snailfish getSnailfish(String line) {
	...
	public Snailfish findSnailfish(Snailfish fish, int dir) {
	...
	public long calculate(Snailfish fish) {
		return fish.isNumber() ? fish.value:(calculate(fish.left) * 3 + calculate(fish.right) * 2);
	}
	
	public Snailfish reduce(Snailfish fish) {
		while (reduce(fish, 0, true) || reduce(fish, 0, false));
		return fish;
	}
	
	public boolean reduce(Snailfish fish, int depth, boolean explode) {
		if (!fish.isNumber()) {
